...
class rfc2119interpretation(nodes.Admonition, nodes.Element):
    pass
...
def purge_rfc2119_mandatory(app, env, docname):
    if not hasattr(env, "rfc2119_all_mandatorys"):
        return
    env.rfc2119_all_mandatory = [
        mandatory
        for mandatory in env.rfc2119_all_mandatory
        if mandatory["docname"] != docname
    ]
...
def purge_rfc2119_recommendation(app, env, docname):
    if not hasattr(env, "rfc2119_all_recommendation"):
        return
    env.rfc2119_all_recommendation = [
        rec for rec in env.rfc2119_all_recommendation if rec["docname"] != docname
    ]
...
def purge_rfc2119_optional(app, env, docname):
    if not hasattr(env, "rfc2119_all_optional"):
        return
    env.rfc2119_all_optional = [
        rec for rec in env.rfc2119_all_optional if rec["docname"] != docname
    ]
